...
export const uploadExcel = async (req, res, next) => {
    try {
        ...
        const candidates = data.map((row) => ({
            ...
            currentAnnualSalary: Number(row['Current Annual Salary'] || row.currentAnnualSalary || row['Current Annual'] || 0),
        }));
...
export const exportExcel = async (req, res, next) => {
    try {
        const { search, experienceMin, experienceMax, location, skills, currentCompany, salaryMin, salaryMax, noticePeriod, industry, } = req.query;
        ...
        if (salaryMin || salaryMax) {
            ...
            if (salaryMin)
                query.currentAnnualSalary.$gte = Number(salaryMin);
            if (salaryMax)
                query.currentAnnualSalary.$lte = Number(salaryMax);
        }
        if (noticePeriod) {
